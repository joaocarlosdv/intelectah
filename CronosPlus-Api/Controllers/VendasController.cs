...
{
    public class VendasController : ControllerCrud<Vendas, VendasDto>
    {
        private readonly IVendasService _vendasService;
        public VendasController(IVendasService service) : base(service)
        {
            _vendasService = service; 
        }
        ...
        public async Task<ActionResult> ConsultaPaginada(int limit = 10, int offset = 0, string? search = null, int colOrder = 0, string dirOrder = "asc")
        {
            try
            {
                return Ok(await _vendasService.ConsultaPaginada(limit, offset, search, colOrder, dirOrder));
            }
            catch (Exception ex)
            {
